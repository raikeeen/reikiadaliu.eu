...
class AdminOrdersController extends AdminOrdersControllerCore
{
        ...
        if (version_compare(_PS_VERSION_, '1.7', '>=') && Tools::isSubmit('submitShippingNumber') && isset($order))
        {
            if ($this->access('edit')) {
                ...
                else
                {
                    ...
                    {
                        // Check if old carrier was LPExpress
                        ...
                        // Check if new carrier is LPExpress
                        ...
                        {
                            // Delete information about order from LPExpress order table
                            $lp_order = new LPOrder();
                            $lp_order->loadByOrderID($order->id);
                            if (Validate::isLoadedObject($lp_order))
                            {
                        ...
                        {
                            // Add information about order to LPExpress order table
                            $lp_order = new LPOrder();
    ...
    private function submitLPOrders($id_orders)
    {
        ...
        // Check if connections with BalticPost available
    ...
    private function callCarrier($id_orders)
    {
        ...
        if (!count($errors) && !empty($parcels))
        {
            ...
            else
            {
                ...
                {
                    // TODO: Maybe zip all file into single file and download?
                    $html = '';
